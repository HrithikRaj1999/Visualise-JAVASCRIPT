...
const at = (type: VisualizerEvent['type'], ts: number, extra: Record<string, unknown> = {}): VisualizerEvent => {
  return { type, ts, ...extra } as VisualizerEvent;
};

export const examples: Record<string, { title: string; learn: string; code: string; events: VisualizerEvent[] }> = {
  timersVsMicrotasks: {
    ...
    events: [
      at('SCRIPT_START', 1),
      at('ENQUEUE_TASK', 2, { queue: 'timers', taskId: 't1', label: 'setTimeout callback' }),
      at('ENQUEUE_MICROTASK', 3, { queue: 'promise', id: 'p1', label: 'Promise.then' }),
      at('DRAIN_MICROTASKS_START', 4),
      at('DEQUEUE_MICROTASK', 5, { queue: 'promise', id: 'p1' }),
      at('DRAIN_MICROTASKS_END', 6),
      at('PHASE_ENTER', 7, { phase: 'timers' }),
      at('DEQUEUE_TASK', 8, { queue: 'timers', taskId: 't1' }),
      at('CALLBACK_START', 9, { taskId: 't1', label: 'setTimeout callback' }),
      at('CALLBACK_END', 10, { taskId: 't1' }),
      at('PHASE_EXIT', 11, { phase: 'timers' }),
      at('SCRIPT_END', 12),
    ],
  },
  immediateVsTimeout: {
    ...
    events: [
      at('SCRIPT_START', 1),
      at('ENQUEUE_TASK', 2, { queue: 'timers', taskId: 't1', label: 'setTimeout callback' }),
      at('ENQUEUE_TASK', 3, { queue: 'check', taskId: 'c1', label: 'setImmediate callback' }),
      at('PHASE_ENTER', 4, { phase: 'timers' }),
      at('DEQUEUE_TASK', 5, { queue: 'timers', taskId: 't1' }),
      at('CALLBACK_START', 6, { taskId: 't1', label: 'setTimeout callback' }),
      ...
      at('CALLBACK_START', 11, { taskId: 'c1', label: 'setImmediate callback' }),
  ...
  nextTickPriority: {
    ...
    events: [
      at('SCRIPT_START', 1),
      at('ENQUEUE_MICROTASK', 2, { queue: 'promise', id: 'p1', label: 'Promise.then' }),
      at('ENQUEUE_MICROTASK', 3, { queue: 'nextTick', id: 'n1', label: 'process.nextTick' }),
      at('DRAIN_MICROTASKS_START', 4),
  ...
  ioVsImmediate: {
    ...
    events: [
      at('SCRIPT_START', 1),
      at('ENQUEUE_TASK', 2, { queue: 'io', taskId: 'io1', label: 'fs.readFile callback' }),
      at('ENQUEUE_TASK', 3, { queue: 'check', taskId: 'c1', label: 'setImmediate callback' }),
      at('PHASE_ENTER', 4, { phase: 'io' }),
      at('DEQUEUE_TASK', 5, { queue: 'io', taskId: 'io1' }),
      at('CALLBACK_START', 6, { taskId: 'io1', label: 'fs.readFile callback' }),
  ...
  closeHandlers: {
    ...
    events: [
      at('SCRIPT_START', 1),
      at('ENQUEUE_TASK', 2, { queue: 'close', taskId: 'cl1', label: 'close handler' }),
      at('PHASE_ENTER', 3, { phase: 'close' }),
      at('DEQUEUE_TASK', 4, { queue: 'close', taskId: 'cl1' }),
      at('CALLBACK_START', 5, { taskId: 'cl1', label: 'close handler' }),
